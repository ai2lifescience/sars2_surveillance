...
if __name__ == '__main__':
    ...
    data = pd.read_csv('/media/data/fuhaoyi/sequencing_data/20230108/'
                       'output/6tree/20230108_nextclade.csv', sep=';')
    ...
    path = Path('../src/ref_lineage_mutation')
    ref_lineage = pd.read_csv(path / 'BF.7.14.csv')
    bf714 = different_mutaion(data, ref_lineage)

    path = Path('../src/ref_lineage_mutation')
    ref_lineage = pd.read_csv(path / 'BA.5.2.48.csv')
    ba5248 = different_mutaion(data, ref_lineage)
    ...
    output_data = pd.DataFrame({
        ...
        'diff_BA.5.2.48': ba5248,
        'diff_BF.7.14': bf714,
        'diff_BA.5.2': ba_5_2,
        'diff_BF.7': bf7,
        'aaSubstitutions':data['aaSubstitutions']
